...
class RDPApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        # Honorar OCR (convenience inside Fees)
        honorar_frame = ttk.LabelFrame(fees_tab, text="Honorar OCR")
        honorar_frame.pack(fill=tk.BOTH, expand=False, pady=6)

        # Keyword (reuse global keyword_var)
        kw_row = ttk.Frame(honorar_frame)
        kw_row.pack(anchor="w", pady=(4, 0))
        ttk.Label(kw_row, text="Keyword").pack(side=tk.LEFT)
        try:
            # If OCR tab already created keyword_var, reuse; else create it now
            self.keyword_var
        except AttributeError:
            self.keyword_var = tk.StringVar(value=self.cfg.get("keyword", "Honorar"))
        ttk.Entry(kw_row, textvariable=self.keyword_var, width=20).pack(
            side=tk.LEFT, padx=6
        )

        # Calibration helpers for Honorar extraction
        cal_row = ttk.Frame(honorar_frame)
        cal_row.pack(anchor="w", pady=(4, 0))
        ttk.Button(
            cal_row, text="Pick Result Region", command=self.pick_result_region
        ).pack(side=tk.LEFT, padx=2)
        ttk.Button(
            cal_row, text="Pick Honorar Region", command=self.pick_amount_region
        ).pack(side=tk.LEFT, padx=2)

        # Toggle to use the saved Honorar region (profile)
        try:
            self.use_profile_var
        except AttributeError:
            self.use_profile_var = tk.BooleanVar(
                value=self.cfg.get("use_amount_profile", False)
            )
        ttk.Checkbutton(
            honorar_frame, text="Use Honorar region (profile)", variable=self.use_profile_var
        ).pack(anchor="w", pady=(4, 0))

        # Helper vars for saving a quick profile named "Honorar"
        try:
            self.new_prof_name_var
        except AttributeError:
            self.new_prof_name_var = tk.StringVar(value=self.cfg.get("active_amount_profile", ""))
        try:
            self.prof_keyword_var
        except AttributeError:
            self.prof_keyword_var = tk.StringVar(value="")

        # Quick test actions (reusing existing logic)
        test_row = ttk.Frame(honorar_frame)
        test_row.pack(anchor="w", pady=(4, 2))
        ttk.Button(
            test_row, text="Test Parse (full region)", command=self.test_parse_full
        ).pack(side=tk.LEFT, padx=2)
        ttk.Button(
            test_row, text="Test Parse (Honorar region)", command=self.test_parse_profile
        ).pack(side=tk.LEFT, padx=2)
        ttk.Button(
            test_row,
            text="Save as 'Honorar' Profile",
            command=lambda: (
                self.new_prof_name_var.set("Honorar"),
                self.prof_keyword_var.set(self.keyword_var.get().strip() or "Honorar"),
                self.save_amount_profile(),
        ...
        ttk.Label(
            honorar_frame,
            text=(
                "Pick Result Region, then pick a Honorar sub-region inside it. "
                "Tests use your current OCR settings and language."
            ),
            wraplength=360,
            justify=tk.LEFT,
        ...
        self.rechnungen_gg_csv_var = tk.StringVar(
            value=self.cfg.get("rechnungen_gg_results_csv", "rechnungen_gg_results.csv")
        )
        ttk.Entry(rechn_frame, textvariable=self.rechnungen_gg_csv_var, width=40).pack(
            anchor="w", pady=(0, 4)
        )
    ...
    def _extract_rechnungen_label_info(self, row, norm):
        ...
        if not label_tokens:
            return {
                "display": "",
                "normalized": normalize_gg_candidate(norm),
                "box": None,
            }
    ...
    def _extract_rechnungen_gg_entries(self, prefix=""):
        ...
        log_lines = []
        if gg_entries:
            log_lines.append(f"{prefix}Detected {len(gg_entries)} GG transaction(s).")
        else:
    ...
    def _wait_for_preview_ready(self, timeout=1.5):
        ...
        if evt is None:
            deadline = (
                None if timeout is None else (time.time() + max(0.0, float(timeout)))
            )
            while True:
    ...
    def pull_form_into_cfg(self):
        ...
        if hasattr(self, "rechnungen_gg_csv_var"):
            self.cfg["rechnungen_gg_results_csv"] = (
                self.rechnungen_gg_csv_var.get().strip() or "rechnungen_gg_results.csv"
            )
    ...
    def _fees_scan_current_page_amount(self):
        ...
        # Wait for region to load and stabilize
        region_wait = float(self.cfg.get("rechnungen_region_wait", 9.0))
        time.sleep(region_wait)  # Wait for document to fully load

        # Take multiple samples to ensure we get the live content
        max_attempts = 3
        for attempt in range(max_attempts):
            ...
            try:
                text = " ".join(
                    [t for t in df["text"].tolist() if str(t).strip()]
                ).strip()
            except Exception:
                text = ""

            if text:  # If we got some content, validate it's not a loading screen
                if not any(
                    loading_indicator in text.lower()
                    for loading_indicator in ["loading", "laden", "bitte warten"]
                ):
                    break

            time.sleep(1.0)  # Short delay between attempts
    ...
    def _fees_open_and_extract_one(self, row_idx, prefix=""):
        ...
        # Additional wait after clicking row to ensure UI is ready
        time.sleep(float(self.cfg.get("rechnungen_region_wait", 9.0)))

        if not self._click_view_button(prefix=prefix):
            return None
        self._fees_overlay_wait("pdf")

        # Extra wait for document to fully load before attempting to read
        time.sleep(float(self.cfg.get("doc_open_wait", 3.0)))
    ...
    def run_fees(self):
        ...
        if not kfb_rows:
            ...
            return
        self.log_print(
            f"{prefix}Found {len(kfb_rows)} KFB entr{'y' if len(kfb_rows)==1 else 'ies'}."
        )
    ...
    def test_fees(self):
        ...
        if pdf_open:
            for _ in self._fees_iter_click_pages(max_clicks=6):
                # Multiple attempts to ensure we get live content
                attempts = 3
                ...
                if amount:  # If we found an amount, stop checking pages
                    break
        else:
            self._click_file_search_and_type_kfb()
            self._fees_overlay_wait("doclist")
